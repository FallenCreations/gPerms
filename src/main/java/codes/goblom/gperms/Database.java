...
public class Database {
     ...
     */
    public Database(GoblomPerms plugin) {
        ...
        plugin.getConfigOption("driver", "com.mysql.jdbc.Driver");
        plugin.getConfigOption("host", "jdbc:mysql://localhost:3306");
        plugin.getConfigOption("user", "root");
        plugin.getConfigOption("pass", "");
        plugin.getConfigOption("isolation", "TRANSACTION_SERIALIZABLE");
        plugin.getConfigOption("logging", false);
        plugin.getConfigOption("rebuild", false);
    }
    
    @Deprecated //Fix getConfigOption to return a default
    public void prepare(String database, List<Class<?>> databaseClasses) {
        
        String driver = plugin.getConfigOption("driver", "com.mysql.jdbc.Driver");
        String url = plugin.getConfigOption("host", "") + "/" + database;
        String username = plugin.getConfigOption("user", "");
        String password = plugin.getConfigOption("pass", "");
        String isolation = plugin.getConfigOption("isolation", "");
        boolean logging = plugin.getConfigOption("logging", false);
        boolean rebuild = plugin.getConfigOption("rebuild", false);

        this.databaseClasses = databaseClasses;
        this.initializeDatabase(driver, url, username, password, isolation, logging, rebuild);
    }
    ...
    private void installDatabase(boolean rebuild) {
        ...
        for (Class<?> classe : classes) {
            try {
                ...
                // does not exist
                ebeanServer.find(classe).findRowCount();
    ...
    private String validateCreateDDLSqlite(String oldScript) {
        try {
            ...
            String newScript = "";
            newScript = scriptLines.stream().map((newLine) -> newLine + "\n").reduce(newScript, String::concat);
